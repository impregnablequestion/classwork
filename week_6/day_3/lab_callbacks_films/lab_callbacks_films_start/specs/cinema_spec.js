...
describe('Cinema', function () {
  ...
  it('should be able to check whether there are no films from a particular year', function () {
    const actual = cinema.checkByYear(2015);
    const expected = false;
    assert.strictEqual(actual, expected);
  });

  it('should be able to check whether all films are over a particular length', function () {
    const actual = cinema.checkOverLength(90);
    const expected = true;
  });

  it('should be able to check whether any films are under a particular length', function () {
    const actual = cinema.checkOverLength(100);
    const expected = false;
    assert.strictEqual(actual, expected);
  });

  it('should be able to calculate total running time of all films', function () {
    const actual = cinema.calculateTotalRunning();
    const expected = 622;
    assert.strictEqual(actual, expected);
  });
